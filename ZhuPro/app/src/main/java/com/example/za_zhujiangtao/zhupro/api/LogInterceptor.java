...
public class LogInterceptor implements Interceptor {
    ...
    private String fileNameA = "service_a.log";
    private String fileNameB = "service_b.log";
    private String fileName = fileNameA;
    ...
    @Override public Response intercept(Chain chain) throws IOException {
        ...
        bean.setUrl(url);
        Log.e("vvvv", "url = "+url);
        bean.setRequestAccount("zzjt");
        bean.setRequestNet(FileUtils.getNetWorkClass());
        bean.setRequestPlatform("android");
        ...
        if (logHeaders) {
            ...
            for (int i = 0, count = headers.size(); i < count; i++) {
                ...
                if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
                    ...
                    request_Headers += name + ": " + headers.value(i)+";";
                }
            }
        ...
        bean.setRequestStartTime(System.currentTimeMillis());
        Response response;
        try {
        ...
        bean.setRequestEndTime(System.currentTimeMillis());
        bean.setResponseCode(response.code()+"");

        if (!response.isSuccessful()){
            bean.setRequestBody(request_Body);
            bean.setRequestHeaders(request_Headers);
        }
        ...
        if (logHeaders) {
            ...
            for (int i = 0, count = headers.size(); i < count; i++) {
                ...
                if (!response.isSuccessful()){
                    response_Headers += headers.name(i) + ": " + headers.value(i)+";";
                }
            }

            if (!response.isSuccessful()){
                bean.setResponseHeaders(response_Headers);
            }
            ...
            if (!logBody || !HttpEngine.hasBody(response)) {
                ...
                Gson gson = new Gson();
                String logString = gson.toJson(bean);
                LogUtils.getmInstance().writeDataToFile(logString+",\n");
